...
const ReaderScreen = ({
  ...
  const { data: readingProgress, isLoading: progressLoading } = useReadingProgress(currentNovelId, currentChapterNumber);
  const updateReadingProgress = useUpdateReadingProgress();
  const [lastScrollPosition, setLastScrollPosition] = useState(0);
  const isLoading = chapterLoading || novelLoading || progressLoading;
  const isManhwa = novel?.type === 'manhwa';
  const [hasAppliedProgress, setHasAppliedProgress] = useState(false);
  ...
  // Apply reading progress when content is loaded
  useEffect(() => {
    if (!progressLoading && readingProgress && !hasAppliedProgress && contentHeight > 0) {
      const targetPosition = contentHeight * readingProgress.progress;
      if (scrollViewRef.current) {
        scrollViewRef.current.scrollTo({ y: targetPosition, animated: false });
        setHasAppliedProgress(true);
      }
    }
  }, [readingProgress, progressLoading, contentHeight, hasAppliedProgress]);

  // Reset progress application when changing chapters
  useEffect(() => {
    setHasAppliedProgress(false);
  ...
  useFocusEffect(
    useCallback(() => {
      return () => {
        ...
        console.log('Current scroll position:', scrollPositionRef.current);
        console.log('Current content height:', contentHeight);
        if (contentHeight > 0 && scrollPositionRef.current > 0) {
          const progress = scrollPositionRef.current / contentHeight;
          console.log('Saving progress on screen exit:', progress);
          ...
          console.log('Not saving - contentHeight or scrollPosition is 0');
        }
      };
    }, [currentNovelId, currentChapterNumber, contentHeight])
  );
